...
import {reqgetVolunteerRank} from '../../../api'
...
export default {
  ...
  methods: {
    initChart() {
      this.chartInstance = markRaw(echarts.init(this.$refs.hot_ref));
      const initOption = {
        title: {
          text: "▎志愿者占比统计",
          left: 20,
    ...
    // },
    async getData() {
      let res=await reqgetVolunteerRank()
      // this.$http.get("https://mock.apipark.cn/m1/4435779-0-default/aa").then((response) => {
      this.allData = res
      this.updateChart();
      // }).catch((error) => {
